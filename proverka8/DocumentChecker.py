...
def remove_letters(image_path= local_to_absolute_path('data/pdf_images')):
    ...
    """
    pytesseract.pytesseract.tesseract_cmd = r'D:\OlegDocAnalyze\urfu_LLM_Documents\Libraries\Tesseract\tesseract.exe'
...
def build_table_after_preprocessing(images_path = local_to_absolute_path('dilate_image')):
    ...
    for iterator in range(1, len(os.listdir(images_path)) + 1):
        ...
        for c in cnts:
            ...
            if w > 120 and h > 40 and w < 3000 and h < 3000:  # Фильтрация по размерам столбцов
                count += 1
...
def main():
    ...
    pdf_path = r'D:\OlegDocAnalyze\urfu_LLM_Documents\proverka8\data\МАГНИТ_МКУ,_ЗАО_БЕЛКА,_Ф_Р_507,_ОП_697,ЛС_1.pdf'  # Укажите путь к своему PDF
